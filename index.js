...
async function start() { try { const { state, saveCreds } = await useMultiFileAuthState(sessionDir); const { version, isLatest } = await fetchLatestBaileysVersion(); console.log(🤖 PEACE-MD using WA v${version.join('.')}, isLatest: ${isLatest});
    ...
    Matrix.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (initialConnection) {
                ...
                const botName = "𝐏ᴇᴀᴄᴇ 𝐌ᴅ 🤍";
                const ownerName = "𝙋𝙀𝘼𝘾𝙀𝙈𝘼𝙆𝙀𝙍💚";
                const time = moment().tz("Africa/Nairobi").format("hh:mm A");
                const date = moment().tz("Africa/Nairobi").format("dddd, MMMM Do YYYY");

                const autoMessage = `

╭───〔 🌟 CONNECTED SUCCESSFULLY 🌟 〕───╮ │ 🤖 BOT NAME : ${botName} │ 👑 OWNER    : ${ownerName} │ 🕰️ TIME     : ${time} │ 📅 DATE     : ${date} │ 🚀 STATUS   : Online & Ready! ╰─────────────────────────────╯

💬 Hello there! ✨ I'm online and fully operational. Type menu to begin exploring magic commands!

🔐 Powered by PEACE ENGINE ⚙️`;
